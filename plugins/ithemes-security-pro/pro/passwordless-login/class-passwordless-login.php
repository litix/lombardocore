...
class ITSEC_Passwordless_Login {
	...
	public function enqueue() {
		...
		if ( $this->should_include_ui( $action ) && empty( $_GET[ self::HIDE ] ) ) {
			...
			wp_enqueue_script( 'itsec-passwordless-login', plugin_dir_url( __FILE__ ) . 'js/login.js', $deps, 5 );
	...
	public function enqueue_profile_scripts() {
		...
		$request = new WP_REST_Request( 'GET', '/wp/v2/users/' . $user->ID );
		$request->set_query_params( [ 'context' => 'edit' ] );
		$response = rest_do_request( $request );

		if ( ! $response->is_error() ) {
			wp_add_inline_script( 'itsec-passwordless-login-profile', sprintf(
				"wp.data.dispatch('%s').receiveCurrentUserId( %d );", 'ithemes-security/core', wp_get_current_user()->ID
			) );
			wp_add_inline_script( 'itsec-passwordless-login-profile', sprintf(
